...
class NOMA_IRS(Env):
    ...
    def __init__(self, num_users=3, num_elements=4, irs_distance=100,
        ...
        self.action_dims = self.num_users + self.num_elements # K + N
        ...
        self.prev_action = np.hstack((self.pk.flatten(),self.phi.flatten()))
        self.prev_rate = 0
        self.optimal = None
        self.opt = None
    ...
    def init_state(self):
        ...
        self.update_powers(np.hstack((self.pk.flatten(),self.phi.flatten())))
        return np.hstack((self.pk.flatten(),self.pk_r.flatten(),self.pk.flatten(),self.phi.flatten())).astype("float32")
    
    def update_powers(self,action):
        self.pk = action[:self.num_users]
        p_tx = self.pk * self.max_transmit_power
        self.phi = action[-self.num_elements:]
        p_rx = np.abs(self.h_BS.conj().T @ np.diag(self.phi*2j*np.pi) @ self.hk)**2 * p_tx
    ...
    def step(self, action):
        ...
        self.done = (self.time_step_cntr == self.max_time_steps) or (reward >= self.opt)
